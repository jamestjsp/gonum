...
func simdGerShapeFixtures() []simdConsumerFixture {
	...
	for _, shape := range [][3]int{
		...
		for i := 0; i < m; i++ {
			x[i*inc] = float32((i%11)-5) * 0.125
			for j := 0; j < n; j++ {
				a[i*lda+j] = float32((i+j)%13-6) * 0.0625
			}
		}
		for j := 0; j < n; j++ {
			y[j*inc] = float32(j%7-3) * 0.25
		}
		original := append([]float32(nil), a...)
		...
		fixtures = append(fixtures, simdConsumerFixture{
			...
			run:   func() { copy(a, original); simdConsumerClearAVX(); impl.Sger(m, n, 0.5, x, inc, y, inc, a, lda) },
...
func TestSIMDGerShapeFixtures(t *testing.T) {
	for _, f := range simdGerShapeFixtures() {
...
func BenchmarkSIMDGerShapes(b *testing.B) {
	for _, f := range simdGerShapeFixtures() {
