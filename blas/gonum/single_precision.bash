...
| gofmt -r 'float64 -> float32' \
\
| gofmt -r 'f64.DotInc(x, y, f(n), f(incX), f(incY), f(ix), f(iy)) -> float32(float64(alpha) + f32.DdotInc(x, y, f(n), f(incX), f(incY), f(ix), f(iy)))' \
| gofmt -r 'f64.DotUnitary(a, b) -> float32(float64(alpha) + f32.DdotUnitary(a, b))' \
\
| sed -e "s_^\(func (Implementation) \)D\(.*\)\$_\1Sds\2_" \
      -e 's_^// D\(.*\)$_// Sds\1 plus a constant_' \
      -e 's_\\sum_alpha + \\sum_' \
      -e 's/n int/n int, alpha float32/' \
      -e 's/return 0/return alpha/' \
      -e '/^func (Implementation)/i\
//\
...
| gofmt -r 'f64.DotUnitary -> f32.DotUnitary' \
| gofmt -r 'f64.ScalUnitary -> f32.ScalUnitary' \
| gofmt -r 'dtrsmLeftVector -> strsmLeftVector' \
| gofmt -r 'dsyrkBlocked -> ssyrkBlocked' \
...
echo Generating sgemm_simd.go
echo -e '// Code generated by "go generate gonum.org/v1/gonum/blas/gonum”; DO NOT EDIT.\n' > sgemm_simd.go
cat dgemm_simd.go \
| gofmt -r 'float64 -> float32' \
| gofmt -r 'simd.Float64s -> simd.Float32s' \
| gofmt -r 'simd.BroadcastFloat64s -> simd.BroadcastFloat32s' \
| gofmt -r 'simd.LoadFloat64s -> simd.LoadFloat32s' \
...
| gofmt -r 'dgemmSerialTransTransBlocked -> sgemmSerialTransTransBlocked' \
| gofmt -r 'dgemmSerialTransTrans -> sgemmSerialTransTrans' \
| gofmt -r 'dgemmSerialNotNot -> sgemmSerialNotNot' \
| gofmt -r 'dgemmSerialTransNot -> sgemmSerialTransNot' \
| gofmt -r 'gemmSIMDDisjoint -> sgemmSIMDDisjoint' \
| sed -e 's_^// dgemmSerialSIMD_// sgemmSerialSIMD_' \
>> sgemm_simd.go

echo Generating ssyrk_simd.go
echo '// Code generated by "go generate gonum.org/v1/gonum/blas/gonum"; DO NOT EDIT.' > ssyrk_simd.go
cat dsyrk_simd.go \
| gofmt -r 'float64 -> float32' \
| gofmt -r 'simd.BroadcastFloat64s -> simd.BroadcastFloat32s' \
| gofmt -r 'simd.LoadFloat64s -> simd.LoadFloat32s' \
