...
func (Implementation) Strsm(s blas.Side, ul blas.Uplo, tA blas.Transpose, d blas.Diag, m, n int, alpha float32, a []float32, lda int, b []float32, ldb int) {
	...
	if useGEMMSIMD && s == blas.Left && n == 1 {
		strsmLeftVector(ul, tA, d, m, alpha, a, lda, b, ldb)
		return
	}
	...
	if useGEMMSIMD && s == blas.Left && alpha == 1 && m >= 128 && n >= 16 && forward && gemmSIMDHardware() {
		strsmLeftBlocked(ul, tA, d, m, n, a, lda, b, ldb)
		return
	}
...
func strsmLeftVector(ul blas.Uplo, trans blas.Transpose, diag blas.Diag, m int, alpha float32, a []float32, lda int, b []float32, ldb int) {
...
func strsmLeftBlocked(ul blas.Uplo, trans blas.Transpose, diag blas.Diag, m, n int, a []float32, lda int, b []float32, ldb int) {
	for start := 0; start < m; start += blockSize {
		end := min(start+blockSize, m)
		Implementation{}.Strsm(blas.Left, ul, trans, diag, end-start, n, 1, a[start*lda+start:], lda, b[start*ldb:], ldb)
		if end == m {
			continue
		}
		if trans == blas.NoTrans {
			sgemmSerial(false, false, m-end, n, end-start, a[end*lda+start:], lda, b[start*ldb:], ldb, b[end*ldb:], ldb, -1)
		} else {
			sgemmSerial(true, false, m-end, n, end-start, a[start*lda+end:], lda, b[start*ldb:], ldb, b[end*ldb:], ldb, -1)
		}
	}
}
...
func (Implementation) Ssyrk(ul blas.Uplo, tA blas.Transpose, n, k int, alpha float32, a []float32, lda int, beta float32, c []float32, ldc int) {
	...
	if useGEMMSIMD && tA != blas.NoTrans && n >= 16 && k >= 16 && ssyrkBlocked(ul, n, k, alpha, a, lda, beta, c, ldc) {
		return
	}
	if tA == blas.NoTrans {
		if ul == blas.Upper {
