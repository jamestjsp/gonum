...
type simdComplexDotCutoffConsumerFixture struct {
	name  string
	run   func()
	check func(testing.TB)
}

func TestSIMDComplexDotCutoffConsumerFixtures(t *testing.T) {
	for _, fixture := range simdComplexDotCutoffConsumerFixtures() {
...
func BenchmarkSIMDComplexDotCutoffConsumers(b *testing.B) {
	for _, fixture := range simdComplexDotCutoffConsumerFixtures() {
...
func simdComplexDotCutoffConsumerClearAVX() {
	if archsimd.X86.AVX() {
		archsimd.ClearAVXUpperBits()
	}
}

func simdComplexDotCutoffConsumerFixtures() []simdComplexDotCutoffConsumerFixture {
	var fixtures []simdComplexDotCutoffConsumerFixture
	impl := Implementation{}
	for _, n := range []int{127, 129, 255, 257, 511, 513, 4096} {
		for _, inc := range []int{1, 3} {
			for _, conjugate := range []bool{false, true} {
				...
				fixtures = append(fixtures, simdComplexDotCutoffConsumerFixture{
					name: fmt.Sprintf("%s/n=%d/inc=%d", name, n, inc),
					run: func() {
						if conjugate {
							simdComplexDotCutoffConsumerClearAVX()
	...
	for _, dimensions := range [][2]int{{8, 129}, {8, 257}, {8, 513}} {
